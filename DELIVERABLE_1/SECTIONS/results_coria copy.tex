...
\subsection{HOSVD+GPR Applied to the ULB Flameless Furnace}
\label{sec:results_ulb}
...
\begin{table}[htbp]
\centering
\caption{NRMSE comparison between HOSVD+GPR and POD+GPR on the ULB furnace test set.}
\label{tab:nrmse_ulb}
\begin{tabular}{lccc}
\toprule
...
HOSVD+GPR outperforms POD+GPR on every thermochemical variable. The largest gains appear for the minor species: NO$_2$ (74.8\%), NH$_2$ (70.6\%), and H$_2$ (58.3\%). Temperature, the smoothest field, still yields a 22.6\% reduction in error.

% [DISCUSS: comment on why the minor species benefit more — you likely have an intuition from looking at the fields (e.g., sharper spatial gradients, stronger sensitivity to $\phi$, larger relative errors in the POD case)]

\FloatBarrier
\begin{figure}[htbp]
    \centering
    % \includegraphics[width=0.88\textwidth]{../IMAGES/RESULTS/ulb_nrmse_comparison.png}
    \caption{[FIGURE: bar chart of NRMSE comparison across all variables. Add when available.]}
    \label{fig:ulb_nrmse}
\end{figure}
\FloatBarrier
...
\begin{figure}[htbp]
    \centering
    % \includegraphics[width=0.88\textwidth]{../IMAGES/RESULTS/ulb_temperature_reconstruction.png}
...
% [DISCUSS: explain why the multilinear structure is a good match for this problem — the parameters $\phi$, $H_2$, $p$ have physically distinct effects on the flow, so separate factor matrices are a natural fit. This is why freezing $\mathbf{U}_{H_2}$ and $\mathbf{U}_p$ works: the model does not need to re-learn their effect during interpolation]

% [DISCUSS: limitations — interpolation is currently only along $\phi$; why was $\phi$ the natural active dimension? What would it take to extend to simultaneous multi-parameter GPR?]

% [DISCUSS: future directions — the poster mentions extending to the JHC burner with more configurations; what does that require in terms of data and what challenges do you anticipate?]
